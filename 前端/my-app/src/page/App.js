...
function App() {
  ...
  function Item ({ item, date,did}){
    ...
    const handlDel=(did)=>{
      ...
      if(acheck===true)
      {
        axios.delete(`http://localhost:80/delete/${did}`)
        .then(response => {
  ...
  return (
    <div className="App">
      <div className="userName" style={{border:'2px'} }>用户名</div>
      <Button href="http://localhost:3000/LogIn" type="primary" className="logIn" >登录</Button>
      <Button href="http://localhost:3000/Register" type="primary" className="logIn" >注册</Button>
